...
```bash
git clone https://github.com/Conless/eprof.git && cd eprof
uv venv --python 3.10
EPROF_USE_PREBUILT=0 pip install --no-build-isolation './python[energy]'
```

`magneton` itself is pure Python and needs only PyTorch — `pip install
./python` is enough to record, match and compare. Energy measurement is an
extra, because it carries a Rust and C++ extension built against CUPTI and
NVML; `EPROF_USE_PREBUILT=0` builds that from this checkout, which is what you
want until it is published. It needs a CUDA toolkit and clang 18 or newer —
see [docs/ARTIFACT_EVALUATION.md](docs/ARTIFACT_EVALUATION.md) for the
toolchain and for `scripts/ae.py doctor`, which checks a machine for it.
