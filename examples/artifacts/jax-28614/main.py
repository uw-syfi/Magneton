...
`jax.scipy.signal.stft` under `vmap` against `torch.stft` on the same signal.
The issue is about the latency gap; this measures where the energy goes, which
is a different question from which one finishes first.
...
def main():
    ...
    for op in ops:
        ...
        # Only worth breaking out when the operation is not the kernel. XLA
        # names a fusion's kernel after the fusion itself, with the dots turned
        # into underscores, and repeating that line says nothing.
