...
`jax.scipy.linalg.expm` picks a Pade approximant by the norm of its argument,
but computed all five and selected one with `lax.select` -- so the four it did
not need were evaluated anyway, matrix multiplications and all. The fix computes
only the chosen branch, with `lax.switch`.

The issue was reported as a latency problem, and it is one. This measures the
other half: how much energy the discarded branches drew. The two are not the
same number, because the wasted work is dense matrix multiplication, which draws
harder than the average of a mixed workload.

    python main.py

Writes per-operation energy for both versions, and what the fix saved.
"""
...
#
# Lifted from the issue: `expm` picks a Padé approximant by the norm of its
# argument. The reported version computed all five and chose one with
# `lax.select`, so four were evaluated and thrown away; the fix computes only
# the chosen branch, with `lax.switch`. Everything above the choice -- the
# approximants themselves -- is identical.
...
# Each entry is a matrix norm that selects a different Pade order. The lower
# orders are where the fix matters most -- Pade-3 discards four branches, while
# Pade-13 was going to do most of the work regardless.
NORMS = [
    ("Pade-3", 0.005),
    ("Pade-5", 0.100),
    ("Pade-7", 0.600),
...
def matrix_with_norm(size: int, target_norm: float):
    """A matrix whose 1-norm is what we asked for, since that is what selects
    the Pade order."""
...
def export_stablehlo(matrix):
    ...
    The claim above is about work that is computed and discarded, and this is
    where it is visible: the before-fix module contains all five approximants
    and a select over them, the after-fix module a switch with one branch
    taken. Generated rather than committed, so it always matches the code in
    this file and the jax that is installed.
    """
