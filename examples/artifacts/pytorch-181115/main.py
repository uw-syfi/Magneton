...
def worker_normal(rank):
    ...
    if not isinstance(profiler, nullcontext):
        ...
        # What the rank spent while it was not doing its own work. The wait is
        # not idleness the profiler has to infer: DDP's all-reduce is a
        # registered operator, c10d::allreduce_, and its NCCL kernel occupies
        # the GPU and draws power for as long as the collective takes. On the
        # balanced run that is a rounding error; on the imbalanced one it is
        # most of the GPU time, and this table says so by name.
