...
def load_power_series(trace_path: str) -> pd.Series:
    ...
    # A sample at ts 0 is not a measurement. The trace gives that timestamp to
    # anything whose start predates the capture -- the process's outermost
    # python frames get it too -- and two of them here were enough to stretch
    # the grid below from 60 rows to 360 million, which reads as a hang.
    samples = [
