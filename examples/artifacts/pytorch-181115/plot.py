...
def process_trace(trace_file):
    ...
    for event in trace:
        # A sample at ts 0 is not a measurement: the trace gives that timestamp
        # to anything whose start predates the capture. Taking one as the
        # earliest reading stretches this series across the whole span between
        # it and the real ones, which are microseconds apart.
