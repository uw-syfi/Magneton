...
def profile_for_comparison(label):
    ...
    Used by compare_variants.py, which runs this once per version of the
    library and matches the two afterwards. The dataflow recording that the
    matcher needs is switched on here and left off in `main`, where nothing
    reads it and it would only cost time and memory.
    """
