...
This example compares the dataflow of GPT2 inference between:
- vLLM: Optimized inference engine with PagedAttention
- HuggingFace Transformers: Standard implementation

The comparison reveals differences in:
- Operation composition and fusion
...
# Configuration
# Overridable so the same comparison can be run on a smaller model, or on a
# GPU that someone else is also using:
#   EPROF_MODEL=gpt2 EPROF_GPU_FRACTION=0.15 python vllm_vs_hf.py
...
def profile_vllm(llm, prompts, sampling_params, devices):
    ...
    Uses lazy compilation (example_inputs=None) so that torch.compile
    happens during the actual generate() call when vLLM provides the
    real inputs with attention metadata.
    """
    print("  Configuring profiler...")
    ...
    # Then run it once with the profiler still stopped.
    #
    # This is what `example_inputs` does for the HuggingFace side, and without
    # the same treatment the comparison measures the wrong thing. The dataflow
    # plugin computes a mean and a standard deviation for every tensor it
    # records -- two GPU reductions each -- and only on a node's first
    # execution. So whichever side records its dataflow inside the measured
    # region pays for the instrumentation in its own numbers: 1285 of vLLM's
    # 2199 kernels, and 77% of its GPU time, were this profiler's metadata
    # rather than the model's work.
...
def compare_outputs(hf_output, vllm_output) -> None:
    ...
    - HuggingFace returns logits for all positions
    - vLLM returns generated text for sampled tokens
    """
    # Extract HF logits
...
def compare_dataflows(hf_prof, vllm_prof, hf_per_node, vllm_per_node):
    ...
    Counting operations says vLLM uses fewer; it does not say which of
    HuggingFace's an ordinary reader should look at to understand why. The
    matcher answers that: it divides both graphs at the tensors they agree on
    and reports the regions between, so a 48-op stretch of HuggingFace lines up
    with the 4 ops vLLM replaced it with.
    """
...
def cost_of(dag, node_ids, per_node):
    ...
    A node's work is recorded under the wrapper frame the FX transform put
    around it, which is `forward_` plus the node's name -- so the DAG and the
    profile join on that, and a subgraph's cost is the sum over its nodes.
    """
...
def report_matched_subgraphs(matches, hf_dag, vllm_dag, hf_per_node, vllm_per_node):
    ...
    The pairs are equivalent by construction -- same tensors in, same tensors
    out -- so the two columns are two implementations of the same computation,
    and the difference between them is what one costs over the other.
    """
    print("\n" + "-" * 80)
    ...
    # Over the union of the nodes, not the sum of the rows. Consecutive regions
    # meet at their cut points and can share more than that where the graph
    # branches, so a node belongs to more than one row and adding the rows up
    # counts its cost twice.
...
def export_results(hf_prof, vllm_prof):
    ...
    The files themselves are written as each run finishes, not here: a result
    has to be read before the next run starts, or libkineto has already reused
    the buffers underneath it.
    """
    print("\nResults written:")
...
def main():
    ...
    # Read this result out NOW, before vLLM starts. libkineto is a process-wide
    # singleton: preparing a second trace resets the activity buffers this
    # result still points at, and its GPU kernels come back with zero
    # timestamps -- silently, so the numbers look plausible and are not.
