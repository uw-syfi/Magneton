...
fn build_libkineto() -> PathBuf {
    ...
    match eprof_utils::device() {
        eprof_utils::Device::Cuda => {
            ...
            let lib = eprof_utils::cupti_lib_dir(&cupti).unwrap_or_else(|| {
                panic!(
                    "found CUPTI under {} but no libcupti in its lib64/ or lib/. \
                     Point EPROF_CUPTI_DIR at an installation that has both the \
