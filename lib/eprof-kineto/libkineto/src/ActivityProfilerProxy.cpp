...
std::optional<std::vector<boost::stacktrace::frame>>(*get_trace_ptr)(uint32_t) = nullptr;

std::unique_ptr<ActivityTraceInterface> ActivityProfilerProxy::stopTrace(
    bool trace_enabled,
      std::optional<std::vector<boost::stacktrace::frame>>(*get_trace)(uint32_t)
) {
  trace_enable = trace_enabled;
  get_trace_ptr = get_trace;
  return controller_->stopTrace();
}

auto TraceEnabled() -> bool { return trace_enable; }
auto GetTrace(uint32_t id)
    -> std::optional<std::vector<boost::stacktrace::frame>> {
  return get_trace_ptr(id);
}
