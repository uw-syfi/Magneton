...
static string defaultTraceFileName() {
  return std::format("/tmp/libkineto_activities_{}.json", processId());
}
...
Config::Config()
    : verboseLogLevel_(-1),
      ...
      activitiesLogUrl_(std::format("file://{}", activitiesLogFile_)),
...
static std::string getTimeStr(time_point<system_clock> t) {
  return std::format("{:%Y-%m-%d %H:%M:%S}", t);
}
...
void Config::printActivityProfilerConfig(std::ostream& s) const {
  ...
  if (hasProfileStartIteration()) {
    ...
    s << "  Trace start time: "
      << std::format("{:%Y-%m-%d %H:%M:%S}", requestTimestamp());
  ...
  s << "  Enabled activities: " << std::format("{}", StringJoin(activities, ","))
    << std::endl;
