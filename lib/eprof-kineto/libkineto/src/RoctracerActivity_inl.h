...
inline const std::string GpuActivity::name() const {
  if (type_ == ActivityType::CONCURRENT_KERNEL) {
    ...
    return std::format("Memset ({})", getGpuActivityKindString(raw().kind));
...
static inline std::string bandwidth(size_t bytes, uint64_t duration) {
  return duration == 0 ? "\"N/A\"" : std::format("{}", bytes * 1.0 / duration);
}
...
inline const std::string RuntimeActivity<roctracerKernelRow>::metadataJson()
  ...
  correlationToGrid[raw().id] = std::format(
      R"JSON(
    [{}, {}, {}])JSON",
      raw().gridX,
      raw().gridY,
  ...
  correlationToBlock[raw().id] = std::format(
      R"JSON(
    [{}, {}, {}])JSON",
      raw().workgroupX,
      raw().workgroupY,
