...
static std::string defaultFileName() {
  return std::format(kDefaultLogFileFmt, processId());
}
...
void ChromeTraceLogger::handleActivity(const libkineto::ITraceActivity& op) {
  ...
  if (external_id != 0) {
    arg_values.append(std::format("\"External id\": {}", external_id));
  }
  ...
  if (op.type() == ActivityType::CONCURRENT_KERNEL && op.linkedActivity() &&
    ...
    if (!input_tensor_starts.empty()) {
      ...
      arg_values.append(
          std::format(" \"{}\": {}", kInTensorsStart, input_tensor_starts));
    }
    if (!output_tensor_starts.empty()) {
      ...
      arg_values.append(
          std::format(" \"{}\": {}", kOutTensorsStart, output_tensor_starts));
    }
    ...
    if (!processGroupName.empty()) {
      ...
      arg_values.append(
          std::format(" \"{}\": {}", kProcessGroupName, processGroupName));
    }
    ...
    if (!processGroupName.empty()) {
      ...
      arg_values.append(
          std::format(" \"{}\": {}", kProcessGroupDesc, processGroupDesc));
    }
    ...
    if (!groupRanks.empty()) {
      ...
      arg_values.append(std::format(" \"{}\": {}", kGroupRanks, groupRanks));
    }
    const auto& dstRank = collectiveRecord->getMetadataValue(kP2pDst);
    const auto& srcRank = collectiveRecord->getMetadataValue(kP2pSrc);
    if (!dstRank.empty()) {
      arg_values.append(std::format(", \"{}\": {}", kP2pDst, dstRank));
    }
    if (!srcRank.empty()) {
      arg_values.append(std::format(", \"{}\": {}", kP2pSrc, srcRank));
    }
  ...
  if (TraceEnabled() && op.correlationId() != 0) {
    uint64_t start_tic = op.getStartTime();
    uint64_t end_tic = op.getEndTime();
    auto result = GetTrace(op.correlationId());
    if (result != std::nullopt) {
      if (!arg_values.empty()) {
        arg_values.append(", ");
      }
      arg_values.append("\"stacktrace\": [\n");
      int i = 0;
      for (const auto &frame: result.value()) {
        ...
        res += '#';
        res += ' ';
        res += boost::stacktrace::detail::to_string_impl{}(frame.address());
        if (i == result->size() - 1) {
          res += "\"\n";
        } else {
          res += "\",\n";
