//! Driving libkineto.
//!
//! libkineto is a C++ library, but only its *interface* is; everything this
//! module does with it -- deciding which activity types to ask for, matching
//! activities back to the events that produced them, assigning forward/backward
//! flows, walking the linked-activity graph -- is bookkeeping over integers and
//! strings. `libkineto/include/kineto_c_api.h` is the C boundary that lets it
//! live here, and the `extern` block below is that header, restated.
...
// Whether a custom backend has claimed the PrivateUse1 device slot. Such a
// backend reuses the CUDA and MTIA activity types, so without this the type
// alone would not say which device an activity ran on. It is a libtorch
// registration, so only C++ can see it -- see include/backend.h.
extern "C" {
