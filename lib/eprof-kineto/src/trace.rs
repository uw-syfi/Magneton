//! A trace: preparing it, handing our events over, and reading back what
//! libkineto collected.
//!
//! ORDERING IS LOAD-BEARING. `pass_events_to_kineto` must run BEFORE
//! `stop_trace`: libkineto merges the CPU buffer with what it collected while
//! ending the trace, so a buffer transferred afterwards is silently dropped and
//! the trace comes back with no GPU activities at all.
...
/// # Safety
/// `convert` must remain callable with `ctx` until replaced.
...
/// Registers, initializes and prepares a trace. `trace_id`, if non-empty,
/// becomes libkineto's request id so a trace can be found again later.
...
/// The metadata key that carries an event's index on the activity we build for
/// it. Pointer identity does not survive libkineto copying or moving an
/// activity, so the index is how they are matched up again afterwards.
const INDEX_KEY: &str = "Ev Idx";

/// Reads the index back out of an activity's metadata JSON. Returns None if it
/// has none, which is how an activity libkineto produced itself is recognised.
...
/// Pairs each backward op with the forward op it came from, and marks the pair
/// as a flow so a trace viewer can draw the link.
...
/// MUST happen before `stop_trace` -- see the module comment.
...
/// Reconnects the activities that come back from `stop_trace` to the events we
/// passed in, and adds an event for every activity kineto produced itself --
/// the GPU kernels and runtime calls.
///
/// There are two mechanisms tying the two sides together. The first is the
/// correlation id: we push a unique integer around an op, and kineto tags the
/// activities it collects during it. That alone loses the nesting, though.
/// A call to `torch.add` produces three activities -- `aten::add`,
/// `cudaLaunchKernel` and the kernel itself -- and by correlation id the last
/// two are both children of `aten::add`, rather than a chain. Kineto's second
/// mechanism, the flow, records that the launch is the flow's start and the
/// kernel shares its id, which recovers the chain.
struct Transfer<'a> {
    arr: &'a mut EventArray,
    privateuse1: bool,
    /// Activity address -> the event it is. Built lazily as activities are
    /// looked up.
...
impl<'a> Transfer<'a> {
    ...
    /// Records the activity on the event it belongs to, so metadata can be
    ...
    fn event_from_activity(&mut self, activity: *const KinetoCActivity) -> i64 {
        ...
        let i = self.arr.push(Event {
            ...
            parent: -1,
            linked: -1,
            // NB: deliberately no activity pointer. Only the activities we
            // passed in are GenericTraceActivity; kineto's own may derive
            // straight from ITraceActivity and are unsafe to write through.
            ..Default::default()
        });
        i as i64
    }
    ...
    fn to_event(&mut self, activity: *const KinetoCActivity) -> i64 {
        ...
        // Until we are very sure we can reassociate the two sides, be
        // defensive: these types are ones we pass in, so an unmatched one means
        // the index metadata did not survive.
        let activity_type = unsafe { kineto_c_activity_type(activity) };
        if matches!(
            activity_type,
...
/// Ends the trace and merges what kineto collected into `arr`. Returns the
/// trace, which the caller owns and must free with `eprof_kineto_trace_free`.
...
/// Writes each event's metadata onto the libkineto activity it was matched to,
/// then drops the activity pointers -- they must not be touched once the trace
/// has been read.
...
/// Ends collection: hands our events over, stops the trace, and merges what
/// kineto collected back in. The two halves are one operation because the
/// transfer has to happen before the stop.
///
/// # Safety
