...
// The CPython tracer, reached as a handle.
//
// A handle rather than a type, so that holding a tracer does not mean including
// <Python.h>: only tracer.cpp needs the interpreter's headers, and pulling them
// into everything that merely starts or stops tracing would spread CPython's
// macros across files that have no business with them.
