...
//!
//! The tracer records enters and exits separately: an enter carries the trace
// key of the callsite, an exit carries only a timestamp. Pairing them is a
//! stack walk, done here rather than while tracing, so the tracing side stays
//! as cheap as possible.
//!
//! Python calls and C calls are replayed independently. A C call never nests
//! inside a python frame we track, so interleaving the two streams onto one
//! stack would mis-pair them.
...
fn replay_one_kind(
    ...
    // A frame already on the stack when profiling started has no thread of its
    // own on record, so it borrows the one from the next event observed on the
    // same python thread -- which, walking backwards, is the previous one seen.
...
/// `enters` is sorted here rather than by the caller.
pub fn replay(
    cache: &mut PyCache,
    arr: &mut EventArray,
...
/// # Safety
/// All pointers must be valid; `enters` is sorted in place.
#[no_mangle]
...
mod replay_tests {
    ...
    fn exits_after_the_last_enter_are_left_to_the_end_of_the_trace() {
        // Exits are drained when a later enter arrives, so trailing ones never
        // get paired and their frames run to end_time instead. That is the
        // right answer -- a frame still open when profiling stopped did run
        // until then -- but it is surprising enough to pin down.
        let mut c = cache_with_two_frames();
