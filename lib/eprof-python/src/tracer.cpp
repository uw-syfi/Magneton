...
// --- The caches ---
// When the tracer enters a frame it wants to do the MINIMAL amount of work,
// otherwise it distorts the profile it is trying to measure. So it does not
// describe the frame; it builds a cheap key and looks that key up. On a hit it
// stores nothing but the resulting TraceKey. Only on a miss does it pay for
// the CPython calls that read a name off a code object or a class off a
// module, and it pays once per distinct callsite for the whole run. During
// post processing the TraceKeys are resolved back into descriptions.
//
// The caches live in eprof-python/src/cache.rs; what is here is the part that
// must touch CPython -- pulling the fields out of a frame or an object and
// handing them over. A miss is reported in a single call carrying everything
// about the callsite, because the GIL is held for the whole of it.
//
// `nn.Module.__call__` is the one frame described differently. Reporting every
// module call as the same line of `module.py` would be useless, so a module
// call is keyed on the module instance and described by its class instead.

// Identity of a frame's code location: the interned filename/name strings and
// the line. The strings are compared by address, which is what makes this
// cheap enough to build on every call.
...
// Source roots to strip off recorded filenames, so a stack reads as
// `torch/nn/modules/module.py` and not the full install path. The list is
// python's to define; the trimming is the cache's to do.
...
void PythonTracer::recordPyCall(ThreadLocalResults &tls, PyFrameObject *frame,
  ...
  const auto key = [&]() -> TraceKey {
    ...
    // A module call's own frame is `nn.Module.__call__`, so its caller is the
    // frame that called into the module. A plain call with no caller (the
    // bottom of a thread's stack) stands in as its own.
    ...
    // By default, CPython stores locals in a "fast" format, with an array of
    // names and an array of values, so frame->f_locals is NULL -- the
    // interpreter has no need to populate it. If those arrays were public we
    // could get at `self` quickly; they are not, and are not stable across
    // versions, so we have to make the interpreter materialize the full dict.
    THPObjectPtr self;
    if (is_module_call) {
...
// --- Post processing ---
// Pairing enters with exits, replaying the stacks and numbering the results is
// the cache's job too -- it is a walk over (key, time, thread) tuples and the
// descriptions are already there. All that is left here is converting the
// timestamps, which needs the C++ clock converter.
