...
//!
//! Two different things are being parented, by two different rules.
//!
//! CPU-side events nest the way calls nest: an event belongs to the innermost
//! event on its thread that was open when it started. A stack per thread is
//! enough to find that.
//!
//! Kineto activities do not nest -- a GPU kernel runs long after the call that
//! launched it returned, often on another thread -- so they are parented by
//! what libkineto records about causation instead:
//!
//!   * a *flow* links a launch to what it launched. The `cudaLaunchKernel`
//!     runtime call is the flow start (`kLinkAsyncCpuGpu`, start=1) and the GPU
//!     kernel carries the same flow id (start=0); the kernel is parented to the
//!     launch.
//!   * failing that, the activity libkineto linked it to -- typically a runtime
//!     call linked to the aten op that made it.
//!
//! A flow wins over a link, because a flow says "this caused that" while a link
//! only says "these were recorded together".
...
/// Non-Kineto nodes keep their original parent. Returns one parent id per input
/// node, in input order.
...
pub struct Materialized {
    pub parents: Vec<i64>,
    /// Effective tids; kineto nodes get their parent's tid (setKinetoTID). The
    /// C++ side writes these back so `_Event` thread ids stay correct.
    pub tids: Vec<u64>,
}

/// Reproduce the full C++ tree construction: the kineto flow/linked merge
/// (setParents), setKinetoTID (kineto nodes inherit their parent's tid, or
/// `current_tid` when an orphan root), then build_tree's per-tid time-containment
/// stack replay. Returns the final parent id and effective tid of every node.
///
/// `current_tid` is the thread id of the caller materializing the trace (the C++
/// side passes `at::RecordFunction::currentThreadId()`); orphan kineto nodes
/// (overhead activities with no flow/linked parent) are placed on that thread,
