...
//
// Rust decides what to record and in what order (driver.rs);
// these are the reads it cannot perform itself. Each one is coarse on purpose:
// a per-field accessor protocol would put an FFI call on the hottest path in
// the system for every scalar, and the input walk is a recursive traversal of
// a tagged union that does not decompose at all.
