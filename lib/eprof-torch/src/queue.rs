//! The queue an active profiler collects into, and the per-thread subqueues
//! under it.
//!
//! A subqueue exists per recording thread, so collecting an op never takes a
//! lock and never touches a cache line another thread is writing: it is
//! appended to whichever subqueue belongs to the thread that ran it. The only
//! shared structure is the map from thread to subqueue, which is locked once
//! per thread rather than once per op.
//!
//! Nothing is ordered during collection. Timestamps come from a raw counter
//! and threads run independently, so the merge at the end is what turns a pile
//! of per-thread streams into one timeline.
...
pub struct Subqueue {
    ...
    pub inputs: OpInputs,
    /// Per op, in capture order: the collective-communication metadata, and the
    /// keyword arguments. Both are already rendered to strings -- an op that
    /// has neither still gets an empty entry, because materialization reads
    /// these positionally alongside the op list.
...
impl Subqueue {
    ...
    /// Turns this thread's captured ops into events on `arr`. Destructive: the
    /// stores are emptied, since nothing reads them twice.
    fn materialize(&mut self, arr: &mut EventArray, convert: &dyn Fn(i64) -> i64) {
        // Plumbing autograd sequence numbers onto their evaluate_function
        // annotations, and stripping the Windows "struct " prefix from
        // AccumulateGrad, are passes over the op list rather than per-op work.
...
pub struct RecordQueue {
    /// Distinguishes this queue from any other that a thread's cache might
    /// still be pointing at. Never reused.
    pub id: u32,
    subqueues: Mutex<HashMap<u64, Box<Subqueue>>>,
    /// The array this run materializes into.
    pub events: *mut EventArray,
    /// Python frame entries, gathered from every subqueue and handed to the
    /// tracer's replay.
...
impl RecordQueue {
    ...
    ///
    /// # Safety
    /// The returned pointer is valid until the queue is destroyed. Subqueues
    /// are boxed and never moved or removed, so growth of the map does not
...
/// The calling thread's subqueue, created on first use. Cached per thread, so
/// the common case costs one comparison rather than a map lookup and a lock.
///
/// # Safety
/// `queue` must be live. The cached pointer is left dangling when a queue is
/// destroyed; the id is checked first, and queue ids are never reused.
...
/// Queue ids must never repeat: a thread caches the subqueue it last used
/// together with the id of the queue it came from, and reuses it only when the
/// ids still match.
static NEXT_QUEUE_ID: std::sync::atomic::AtomicU32 = std::sync::atomic::AtomicU32::new(0);

/// # Safety
/// `events` must come from `eprof_events_create`; the queue does not own it.
...
/// # Safety
/// `queue` must come from `eprof_queue_create` and not be used after.
...
/// # Safety
/// `queue` must come from `eprof_queue_create`.
...
/// The subqueue for `tid`, created on first use. Valid until the queue is
/// destroyed, which is what lets the caller cache it per thread.
///
/// # Safety
/// `queue` must come from `eprof_queue_create`.
...
///
/// # Safety
/// `sq` must be valid; both strings NUL-terminated. `is_kwinput` selects which
/// of the two lists it joins.
#[no_mangle]
...
/// Merges every subqueue into the event array: ops, the POD streams, and the
/// python frame entries the tracer will replay. `convert` turns a raw clock
/// reading into trace time; C++ owns the clock, so it comes over as a callback.
///
/// # Safety
/// `queue` must come from `eprof_queue_create`; `convert` callable with `ctx`.
...
/// The gathered python frame entries, for the tracer's replay. Writes the count
/// to `out_len`; the pointer is valid until the queue is destroyed.
///
/// # Safety
/// `queue` must be valid; `out_len` writable.
#[no_mangle]
...
/// Appends a python frame entry that did not come from a subqueue -- the frames
