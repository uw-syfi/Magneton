...
//!
//! Power is sampled per device, not per kernel, so a kernel's energy has to be
//! reconstructed: integrate board power over the kernel's window, and where
//! several kernels overlap, split the power between them evenly. Even splitting
//! is a choice, not a measurement -- the hardware does not say which kernel drew
//! what -- but it conserves total energy, which is the property that makes the
//! per-op numbers add up to the run.
//!
//! A kernel is charged to an operator through the correlation id kineto stamps
//! on both. Nothing here touches python or torch types, so it can be tested on
//! synthetic traces.
