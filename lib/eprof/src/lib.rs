...
//!
//! This file is the surface: `_Profiler` to configure and run one,
//! `_ProfilerResult` to read what it collected, and the post-processing that
//! runs over a finished result. Everything it exposes is either a thin call
//! into `driver` or a conversion of already-collected data into python objects
//! -- no measurement happens here, and nothing on a hot path passes through.
//!
//! `_TreeNode` and `export_raw_nodes` are two views of the same materialized
//! tree: one nested for reading, one flat for `eprof.attribution` to compute
//! over. Both are built from the same pre-order export, so they cannot
//! disagree.
...
/// An event's tag, as the name python knows it by. Must agree with
/// `eprof.treediff._EVENT_TYPE_TAGS`, which compares traces by these strings.
...
/// A node of the materialized event tree, duck-typed for eprof.treediff
/// (`name`, `tag`, `start_time_ns`, `duration_time_ns`, `correlation_id`,
/// `children`).
#[pyclass(name = "_TreeNode")]
struct TreeNode {
    #[pyo3(get)]
    name: String,
    /// One of TorchOp, Power, Allocation, OutOfMemory, PyCall, PyCCall,
    /// Kineto, Unknown.
    #[pyo3(get)]
    tag: String,
    #[pyo3(get)]
...
/// What a run can be asked to collect. The numbering is API -- torch's
/// profiler passes these values through -- so entries may be added but not
/// renumbered.
...
impl ProfilerResult {
    ...
    /// The materialized event tree as nested nodes. The export is pre-order,
    /// so a node's parent is always built before it -- one pass, no stack.
    ...
    ///
    /// Keys: id, parent_id, tag, name, start_tid, forward_tid, start_ns,
    /// dur_ns, correlation_id, device, device_type, device_index, power_usage,
    /// resource, flow_id, flow_type, flow_start, linked_correlation, linked_id,
    /// activity_type.
    ///
    /// `parent_id` is -1 on a root, and `power_usage` is -1 on anything that is
    /// not a power sample. This is what magneton.attribution consumes.
...
///
/// `_Profiler` starts a sampler of its own and drains it into the run. This is
/// the same sampler with nothing attached, for measuring a framework the torch
/// callbacks cannot see -- JAX, or anything else that reaches the GPU without
/// going through the dispatcher. What to do with the samples is then the
/// caller's problem, which is the point.
///
/// Timestamps are Unix nanoseconds, so they can be lined up with whatever clock
/// the other framework's trace is on.
...
impl EnergySampler {
    ...
    /// Stops sampling and returns everything collected, as
    /// `(unix_time_ns, device_index, power_mw)`.
    ///
    /// The device index is the position in the list this was constructed with,
    /// not the NVML index, so it matches the device numbering a framework
    /// reports under CUDA_VISIBLE_DEVICES.
...
/// One profiling run, and the power sampler that runs alongside it. The two
/// are independent: the sampler knows nothing about the run, and the run only
/// learns of it when the samples are drained in `stop`.
...
impl Profiler {
    ...
    fn new(
        ...
        // Panics if `activities` is empty: there would be nothing to collect.
        // Any iterable of _ActivityType -- torch's profiler passes a set, and
        // callers reasonably pass a list or tuple.
    ...
    /// `scopes` is accepted because torch's profiler passes it, but a run
    /// always instruments the same RecordFunction scopes.
    ...
    fn stop(&self) -> PyResult<ProfilerResult> {
        ...
        if let Some(mut e) = self.energy.borrow_mut().take() {
            ...
            // A sample is labelled by its position in the device list the
            // caller asked for, not by the NVML index it was read from. NVML
            // enumerates every physical GPU and ignores CUDA_VISIBLE_DEVICES,
            // while a kernel is stamped with the index torch uses -- so under
            // any pinning the two disagree, nothing matches, and the energy
            // attributed to every operator is silently zero. With the list
            // given in visible order the position is torch's index, and with
            // no pinning it is the NVML index unchanged.
    ...
    /// Turns an activity on or off mid-run. Only CPU and CUDA are supported;
    /// anything else is reported and skipped. Toggling one of the two alone
    /// leaves CPU/CUDA correlation incomplete, which is also reported.
...
// --- Post-processing ---------------------------------------------------------
// Attribution and materialization, exposed so that python can run them over a
// result it already holds -- from a trace loaded off disk, or after editing the
// node list. Both are pure functions over data, and take the same shapes the
// profiler's own pipeline uses.

/// Charge latency and energy to the operators that caused them.
///
/// Takes what `eprof/attribution.py` pulls out of a result:
///   cpu_ops: list[(name, cpu_duration_ns)]
///   kernels: list[(start_ns, end_ns, device, correlation_id)]
///   power:   list[(time_ns, device, power_mw)]
///   corr_to_op: dict[correlation_id -> op_name]
///   device_index: optional device filter
///
/// Returns list[(op_name, num_calls, num_kernels, cpu_time_ns, gpu_time_ns,
/// gpu_energy_j)] sorted by energy desc.
#[pyfunction]
...
///
/// `nodes` is one tuple per event-tree node:
///   (id, orig_parent_id, tag, flow_id, flow_type, flow_start, linked_id)
/// Returns the recomputed parent id per node, in input order. Non-Kineto nodes
/// keep their original parent.
#[pyfunction]
...
///
/// `nodes` is one tuple per node:
///   (id, tag, start_tid, forward_tid, start_ns, end_ns,
///    flow_id, flow_type, flow_start, linked_id)
/// Returns the final parent id of every node, in input order.
#[pyfunction]
...
/// The profiler: starting a run, reading what it collected, and the
/// post-processing over it.
///
/// This is the Rust/C++ half. `magneton` is the python layer on top, and is
/// what most callers want; the underscored names here are the raw surface.
#[pymodule]
