...
The point of these is that nothing downstream of the DAG knows which framework
produced it. If a jaxpr and an FX graph both come back as DataflowDAGs, the
matcher compares them by tensor value and never learns that one side calls an
operation `dot_general` and the other `aten::matmul`.
"""
...
def test_the_graph_has_edges():
    ...
    dlpack builds a new torch tensor on every call, and the DAG joins one
    node's output to the next node's input by `id()`. Converting per node
    rather than caching would give a graph of the right shape with nothing
    connecting it, and the matcher would find no spine to cut on.
    """
    x = jnp.ones((8, 8)) * 0.5
...
def test_profiling_jax_after_torch_says_so_rather_than_returning_nothing():
    ...
    A JAX capture after a torch one still runs and still writes a trace, with
    no kernels in it -- which reads as a program that did no GPU work. The
    failure has to be loud because the quiet version is indistinguishable from
    a real measurement.
    """
...
def test_a_backend_measuring_the_wrong_framework_is_refused():
    ...
    A torch backend on a jax program profiles nothing it recognises: the
    annotations never match, every operation reports costing zero, and that is
    indistinguishable from a real measurement of a program that did no work.
    """
