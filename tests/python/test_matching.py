...
Built on hand-made DAGs where the answer is known by inspection, because on a
real trace of two thousand nodes there is nothing to check the result against.

The shape being tested throughout: two graphs computing the same thing by
different routes, sharing tensors at a few points. Those shared points are the
cuts, and what lies between successive cuts is what the matcher should report.
"""
...
def dag(spec) -> DataflowDAG:
    ...
    Tensors are real, because the matcher compares them; a tensor id is its
    identity, so each is created once and shared by reference.
    """
    d = DataflowDAG()
    pool = {}
...
def test_the_spine_does_not_depend_on_node_numbering():
    """A subgraph keeps the ids the full graph gave it, so the tree cannot
...
def two_chains():
    ...
    g1:  0 -> 1 -> 2 -> 3 -> 4        five ops
    g2:  0 -> 1 -> 2                  three ops

    g1 node 1 and g2 node 0 both produce tensor 15; g1 node 3 and g2 node 1
    both produce 20; the last of each produces 25. Three agreements, so three
    cuts, so two regions between them -- n cuts bound n-1 regions.
    """
...
def test_recursion_is_bounded_when_the_cuts_stop_dividing():
    """Two cuts that bracket the whole graph divide nothing. The matcher has to
    notice rather than recurse on itself forever."""
    g1 = dag([(0, [10], [11]), (1, [11], [20])])
...
def test_complex_tensors_are_compared_by_magnitude():
    ...
    A dtype in neither the float nor the integer list fell through to the final
    `return True`, so two complex results matched whatever they were put next
    to -- and `torch.linalg.eigvals` returns complex.
    """
...
def test_the_configured_tolerance_is_the_one_used():
    ...
    its own default, so a comparison that needed more room could not ask."""
...
def test_an_unused_entry_does_not_hide_the_graph():
    ...
    A graph with several independent inputs numbers them 0, 1, 2, and the
    operation consuming them may take 1 and 2 and never 0. Starting the walk at
    0 then reaches nothing and every node is filtered out as unreachable, which
    is how a four-node graph came back with an empty dominator path.
    """
...
def test_integer_tensors_of_the_same_size_but_different_shape():
    """Equivalence is by element count, so these are comparable -- and comparing
    them as they are makes `ne` broadcast and raise instead of answering."""
    from magneton.matching.tensor import tensors_equivalent
