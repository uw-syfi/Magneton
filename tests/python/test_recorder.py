...
The point of these is the separation: magneton records what a model computed
and matches two of those recordings, and measurement is something a backend
supplies. Recording with no backend at all has to work, because that is what
comparing two implementations for *correctness* looks like -- no profiler, no
CUPTI, no NVML, just the graph and the tensors.
"""
...
def test_recording_needs_no_backend():
    """Structure without cost. A comparison built from this can say the two
    sides compute the same thing, which is the whole of correctness checking."""
    x = _inputs("cuda")
...
class Fake:
    ...
    It implements the protocol structurally: no base class, and no import of
    anything from magneton. If this stops working, a backend can no longer be
...
def test_the_warm_up_happens_before_the_backend_starts():
    """The dataflow recorder computes tensor statistics on a node's first
    execution. Paying for that inside the measured region is what once made a
    faster system look eight times slower."""
    order = []
...
def test_a_get_attr_node_survives_the_pass():
    """`get_attr` is not an operation, so the pass has nothing to instrument --
    but it still has to copy it.

    Dynamo emits one for the subgraph a higher-order op calls, which is how an
    autograd.Function is represented. Dropping it produces a graph that looks
    fine and cannot run: megatron's decoder came back as "UnboundLocalError:
    local variable 'fwd_body_0' referenced before assignment", which reads like
    a torch bug and was this pass losing a node.
    """
