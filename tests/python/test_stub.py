...
`lib/eprof/magneton_eprof.pyi` is hand-written, so nothing makes it follow
`lib/eprof/src/lib.rs` except this. Adding a `#[pyfunction]` or a `#[pymethods]`
entry without adding it to the stub fails here, which is the point: a stub that
has quietly fallen behind is worse than no stub, because a type checker believes
it.

Only names are checked, not types -- a signature mismatch is beyond what
introspection can see through PyO3, which reports every method as taking
`*args, **kwargs`.
"""
...
def test_the_stub_ships_with_the_module():
    """PEP 561: a type checker only looks beside the module, and only if the
    package says it is typed."""
